...
function saveData(key) {
    ...
    const now = Date.now();
    localStorage.setItem('qr_pass_last_updated', now.toString());
    
    // Trigger background auto sync push
    if (typeof triggerAutoSyncPush === 'function') {
        triggerAutoSyncPush();
    }
}
...
// ==========================================
// 12. GITHUB DATA SYNCHRONIZATION
// ==========================================
const SYNC_FILE_PATH = 'data_sync.json';
...
function setupSyncSettings() {
    ...
    const saveBtn = document.getElementById('btn-save-sync-settings');
    ...
    const guideContent = document.getElementById('sync-guide-content');
    const guideArrow = document.getElementById('sync-guide-toggle-arrow');
    ...
    // Load saved settings
    const savedUsername = localStorage.getItem('qr_pass_sync_username') || '';
    const savedRepo = localStorage.getItem('qr_pass_sync_repo') || '';
    const savedToken = localStorage.getItem('qr_pass_sync_token') || '';
    const syncEnabled = localStorage.getItem('qr_pass_sync_enabled') === 'true';
    ...
    function updateSyncUI() {
        const hasConfig = localStorage.getItem('qr_pass_sync_token');
        if (hasConfig && syncEnabled) {
            if (setupPanel) setupPanel.classList.add('hidden');
            if (actionsPanel) actionsPanel.classList.remove('hidden');
            if (badge) {
                badge.className = 'sync-badge badge-green';
                badge.textContent = '同期有効';
            }
        } else {
            if (setupPanel) setupPanel.classList.remove('hidden');
            if (actionsPanel) actionsPanel.classList.add('hidden');
            if (badge) {
                badge.className = 'sync-badge badge-gray';
    ...
    if (saveBtn) {
        saveBtn.addEventListener('click', async () => {
            ...
            if (badge) {
                badge.className = 'sync-badge badge-yellow';
                badge.textContent = '接続テスト中...';
            }
            saveBtn.disabled = true;

            try {
                // Test connection & initial sync
                const result = await syncWithGitHub(username, repo, token, true); // force pull/push test
                if (result) {
                    localStorage.setItem('qr_pass_sync_username', username);
                    localStorage.setItem('qr_pass_sync_repo', repo);
                    localStorage.setItem('qr_pass_sync_token', token);
                    localStorage.setItem('qr_pass_sync_enabled', 'true');
                    
                    alert('GitHubとの連携・初回同期に成功しました！');
                    updateSyncUI();
                } else {
                    alert('連携テストに失敗しました。ユーザー名、リポジトリ名、トークン、または権限を確認してください。');
                    if (badge) {
                        badge.className = 'sync-badge badge-red';
                        badge.textContent = '接続エラー';
                    }
                }
            } catch (err) {
                console.error(err);
    ...
    if (syncNowBtn) {
        syncNowBtn.addEventListener('click', async () => {
            const username = localStorage.getItem('qr_pass_sync_username');
            const repo = localStorage.getItem('qr_pass_sync_repo');
            const token = localStorage.getItem('qr_pass_sync_token');
            ...
            try {
                const success = await syncWithGitHub(username, repo, token, false);
                if (success) {
                    if (badge) {
                        badge.className = 'sync-badge badge-green';
                    ...
                    setTimeout(() => {
                        if (localStorage.getItem('qr_pass_sync_enabled') === 'true' && badge) {
                            badge.className = 'sync-badge badge-green';
                            badge.textContent = '同期有効';
                        }
                    }, 3000);
    ...
    if (disconnectBtn) {
        disconnectBtn.addEventListener('click', () => {
            if (confirm('GitHubとの連携を解除しますか？ローカルのデータは削除されません。')) {
                localStorage.removeItem('qr_pass_sync_username');
                localStorage.removeItem('qr_pass_sync_repo');
                localStorage.removeItem('qr_pass_sync_token');
                localStorage.removeItem('qr_pass_sync_enabled');
...
// Global hook to trigger sync on data change
async function triggerAutoSyncPush() {
    const syncEnabled = localStorage.getItem('qr_pass_sync_enabled') === 'true';
    if (!syncEnabled || isSyncingInProgress) return;
    ...
    if (badge) {
        badge.className = 'sync-badge badge-yellow';
        badge.textContent = '自動保存中...';
    }

    try {
        await syncWithGitHub(username, repo, token, false);
        if (badge) {
            badge.className = 'sync-badge badge-green';
            badge.textContent = '同期完了';
            setTimeout(() => {
                if (localStorage.getItem('qr_pass_sync_enabled') === 'true') {
                    badge.className = 'sync-badge badge-green';
                    badge.textContent = '同期有効';
                }
            }, 2000);
...
// Base64 UTF-8 helpers (for safe Japanese text encoding in GitHub API)
...
// GitHub Sync core function
async function syncWithGitHub(username, repo, token, isInitialTest = false) {
    const url = `https://api.github.com/repos/${username}/${repo}/contents/${SYNC_FILE_PATH}`;
    const headers = {
    ...
    // 1. Fetch remote data from GitHub
    try {
        const resp = await fetch(url, { headers });
        if (resp.status === 200) {
            const result = await resp.json();
            remoteSha = result.sha;
            const jsonText = decodeBase64Utf8(result.content.replace(/\s/g, ''));
            remoteData = JSON.parse(jsonText);
            fileExists = true;
        } else if (resp.status === 404) {
            // First time sync, remote file does not exist yet
            fileExists = false;
        } else {
    ...
    // 2. Prepare local data with timestamp
    let lastUpdated = parseInt(localStorage.getItem('qr_pass_last_updated') || '0', 10);
    if (lastUpdated === 0) {
        lastUpdated = Date.now();
        localStorage.setItem('qr_pass_last_updated', lastUpdated.toString());
    }
    ...
    const localData = {
        ...
        settings: {
            ...
            notifications: state.settings?.notifications || { attendReminders: true }
        },
    ...
    if (fileExists && remoteData) {
        ...
        if (remoteUpdated > lastUpdated) {
            // Remote is newer - merge and update local
            needUpdateLocal = true;
            dataToSave = mergeSyncData(localData, remoteData);
            // If the merge result has elements not in remote, we also need to push it back
            needUpdateRemote = true; 
        } else if (remoteUpdated < lastUpdated || isInitialTest) {
            // Local is newer (or initial setup upload) - push local to remote
            needUpdateRemote = true;
            dataToSave = mergeSyncData(remoteData, localData);
        } else {
            // Same timestamp - ensure histories are merged just in case
            dataToSave = mergeSyncData(localData, remoteData);
            if (JSON.stringify(localData.history) !== JSON.stringify(dataToSave.history)) {
                needUpdateLocal = true;
                needUpdateRemote = true;
            }
        }
    } else {
        // Remote file does not exist - must create it with local data
        needUpdateRemote = true;
    }

    // 3. Update Local Storage if needed
    if (needUpdateLocal) {
        state.classes = dataToSave.classes;
        state.history = dataToSave.history;
        if (dataToSave.settings) {
            state.settings = dataToSave.settings;
            if (dataToSave.settings.periods) {
                state.periods = dataToSave.settings.periods;
            }
        }
        
        localStorage.setItem('qr_pass_classes', JSON.stringify(state.classes));
        localStorage.setItem('qr_pass_history', JSON.stringify(state.history));
        if (state.periods) {
            localStorage.setItem('qr_pass_periods', JSON.stringify(state.periods));
        }
        localStorage.setItem('qr_pass_settings', JSON.stringify(state.settings));
        localStorage.setItem('qr_pass_last_updated', dataToSave.lastUpdated.toString());
        ...
        renderTimetableForCurrentTab();
        renderHistory();
        renderPeriodsInSettings();
    }

    // 4. Update Remote GitHub file if needed
    if (needUpdateRemote) {
        // Ensure remote SHA is completely fresh before saving to prevent 409 Conflict
        if (fileExists) {
            try {
                const freshResp = await fetch(url, { headers });
                if (freshResp.status === 200) {
                    const freshResult = await freshResp.json();
                    remoteSha = freshResult.sha;
                }
            } catch (err) {
                console.error('Failed to refresh remote SHA:', err);
            }
        }

        // Increment timestamp to make it sync master
        const pushTimestamp = Math.max(dataToSave.lastUpdated, Date.now());
        dataToSave.lastUpdated = pushTimestamp;
        localStorage.setItem('qr_pass_last_updated', pushTimestamp.toString());

        const body = {
            message: 'Update QR-Pass sync data [skip ci]',
            content: encodeBase64Utf8(JSON.stringify(dataToSave, null, 2)),
        };
        if (remoteSha) {
            body.sha = remoteSha;
        }
        ...
        try {
            ...
            if (resp.status !== 200 && resp.status !== 201) {
                console.error('GitHub API PUT error:', resp.status, await resp.text());
                return false;
            }
        } catch (err) {
            console.error('GitHub API save failed:', err);
            return false;
        }
    }
...
// Merge data logic (Precedence: newer whole data for structure, Union for history logs)
function mergeSyncData(oldData, newData) {
    const merged = {
        // Classes are replaced by the newer configuration
        classes: newData.classes || oldData.classes || [],
        settings: newData.settings || oldData.settings || {},
        lastUpdated: Math.max(oldData.lastUpdated || 0, newData.lastUpdated || 0)
    };

    // History logs are merged by unique IDs (timestamp or classId+timestamp combo)
    const historyMap = new Map();
    const oldHistory = oldData.history || [];
    const newHistory = newData.history || [];

    // Map keys by timestamp (since each attendance log has a unique timestamp)
    ...
    // Convert map back to sorted array (newest attendance first)
    merged.history = Array.from(historyMap.values()).sort((a, b) => b.timestamp - a.timestamp);
